    ...
    <section id="blog" class="blog">
      ...
      <div v-if="filteredPosts.length" class="blog__container">
        ...
        <transition-group class="blog__wrapper" mode="out-in" name="blog-bottom" tag="div">
          <BlogCard v-for="post in filteredPosts" :key="post.node.id" :post="post" />
        </transition-group>
      </div>
...
export default {
  ...
  computed: {
    ...
    filteredPosts() {
      return this.$page.Blog.edges.filter(
        (item) => item.node.category.includes(this.activeCategory) && item.node.date_published <= this.actualDate
      );
    },
  },
  mounted() {
