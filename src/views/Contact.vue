<template>
  <section id="Contact">
    ...
    >
      ...
      />
      <transition name="fade">
        <span class="error" v-if="$v.title.$dirty && !$v.title.required"
          >title is required.</span
        >
        <span class="error" v-if="$v.title.$dirty && !$v.title.minLength"
      ...
      <transition name="fade">
        ...
        >
        <span class="error" v-if="$v.message.$dirty && !$v.message.minLength"
          >title must have at least
          {{ $v.message.$params.minLength.min }} letters.</span
        >
      </transition>
      <button class="form__submit" type="submit" :disabled="$v.$invalid">
        send
      </button>
    </form>
    <footer class="footer">
...
.form {
  ...
  &__input {
    ...
    &.mail {
      background: url("../assets/inputs/Email_ico.svg") no-repeat scroll 10px
        12px;
      background-size: 20px 20px;
    }

    &.title {
      background: url("../assets/inputs/Title_ico.svg") no-repeat scroll 9px
        11px;
      background-size: 23px 23px;
    }

    &.message {
      background: url("../assets/inputs/Message_ico.svg") no-repeat scroll 11px
        13px;
